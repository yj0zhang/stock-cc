...
import echarts from "echarts12/dist/weapp/components/echart/echarts"
...
const offlineOptions: Function =  (lineData: Array<IlineNode>) => {
  ...
  for(let i = 0,len = lineData.length; i < len; i++) {
      ...
      if (i + highLightNum < len) {
          highLightStartDate = lineData[i].dateTime;
      }
      if (i == len - 1) {
          highLightEndDate = lineData[i].dateTime;
      }
  }
